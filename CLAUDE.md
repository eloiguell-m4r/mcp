...
- **DOMINI estricte (evitar al·lucinació de categories).** El servidor declara `title` + `instructions` (a `src/server.ts`, `SERVER_INFO`/`SERVER_INSTRUCTIONS`) que fixen que Motion4Rent lloga **ajudes de mobilitat** (cadires de rodes manuals/elèctriques, scooters de mobilitat, knee scooters, caminadors) i **NO** bicicletes, e-bikes, motos ni cotxes. Bug observat en proves de connector: en presentar-lo, Claude deia "scooters **and bikes**" (extrapolava d'una descripció genèrica). Regla clau propagada a `instructions` + descripció de `search_mobility_rentals` + `note` dels resultats: **no afirmar cap categoria fins que una cerca la retorni**; mostrar només les de `product_types_available`/`products`; mai dir que ofereix bikes. Quan es demana un `product_type` inexistent a la ciutat/dates, `search` retorna `requested_type_available:false` + avís al `summary` (dir-ho clarament i no reetiquetar altres tipus). ⚠️ La targeta del **directori públic** (un cop publicat) usa la descripció que s'hi enviï → cal escriure-la també precisa (mateix domini), no depèn del codi.
