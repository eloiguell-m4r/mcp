...
- **HORES vs DATES (exclusió per horari, no recàrrec).** L'API `/search/results` compara pickup/return amb l'horari de `stores_schedule`: una hora **fora d'horari EXCLOU la botiga** (0 resultats), amb un **sostre dur de ~20:00** (i terra 08:00). ≠ del `closed_service` (que és un recàrrec, i només aplica dins 08:00–20:00 o dies tancats). Per tant una **devolució a les 23:00 buida el resultat** encara que la data tingui stock. Fix a `search_mobility_rentals` (`tools.ts`): quan la cerca surt buida amb hores no estàndard, fa **un probe a 10:00/10:00**; si aquell té stock → `available_at_requested_times:false` + `time_fallback_used:true`, ensenya els resultats a hores estàndard i el `summary`/`note` diuen que la DATA té stock però l'hora demanada no és possible (mai "no availability for these dates"). El `bookingLink` ara passa `sh`/`eh` **efectius** (abans sempre 10:00 → incoherent amb la cerca a 23:00). L'API NO es toca (excloure fora d'horari és correcte de negoci).
- **HORARI real de botiga (última devolució / primera recollida exactes).** Per no endevinar l'hora (bug: l'assistent provava hores i deia "fins a les 19:00" quan el web permet fins a les 20:00), `search_mobility_rentals` i `get_rental_details` exposen `earliest_pickup_time`/`latest_return_time` (i `store_hours` a details) per als dies de la reserva. Font: `getStoreHours()` (m4rApi.ts) → `GET /stores/schedule/:id_store` (ja existent), cachejat per botiga (TTL 30 min); `openCloseForDate()` calcula obertura/tancament del dia (dies `sunday..saturday` per `getDay()`, files actives all_year/rang, min-open/max-close entre slots). Reporta el tancament REAL (no un sostre 20:00). `null` si no es pot llegir → l'assistent no endevina. Cap tool nova, cap canvi a l'API.
...
- **DOMINI estricte (evitar al·lucinació de categories).** El servidor declara `title` + `instructions` (a `src/server.ts`, `SERVER_INFO`/`SERVER_INSTRUCTIONS`) que fixen que Motion4Rent lloga **ajudes de mobilitat** (cadires de rodes manuals/elèctriques, scooters de mobilitat, knee scooters, caminadors) i **NO** bicicletes, e-bikes, motos ni cotxes. Bug observat en proves de connector: en presentar-lo, Claude deia "scooters **and bikes**" (extrapolava d'una descripció genèrica). Regla clau propagada a `instructions` + descripció de `search_mobility_rentals` + `note` dels resultats: **no afirmar cap categoria fins que una cerca la retorni**; mostrar només les de `product_types_available`/`products`; mai dir que ofereix bikes. Quan es demana un `product_type` inexistent a la ciutat/dates, `search` retorna `requested_type_available:false` + avís al `summary` (dir-ho clarament i no reetiquetar altres tipus). ⚠️ La targeta del **directori públic** (un cop publicat) usa la descripció que s'hi enviï → cal escriure-la també precisa (mateix domini), no depèn del codi. **Text a punt per enganxar (tagline/descripció amb exclusió, casos d'ús): `docs/listing-directori.md`** (secció Notes → "DOMINI").
